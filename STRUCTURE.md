...
| `RaceLink_Host/racelink/domain/network_boundary.py` `validate_scene_scope_consistency` + `SceneScopeViolation` (repository-coupled cross-validator at the API layer) | `docs/RaceLink_Host/architecture.md` §"Cross-network fan-out" violation table; `docs/RaceLink_Host/operator-guide.md` §7 if the rejection messages change |
| `RaceLink_Host/frontend/src/components/scenes/SceneNetworkScopeWidget.vue` (scope picker chip + dialog) | `docs/RaceLink_Host/operator-guide.md` §7 "Scope chip" subsection; `docs/RaceLink_Host/multi-network.md` §"Scene broadcast scope" UI parts |
| `RaceLink_Host/frontend/src/components/scenes/SceneTargetPicker.vue` / `MultiGroupPickerDialog.vue` (cascading scope filter for per-action target dropdowns) | `docs/RaceLink_Host/operator-guide.md` §7 cascading-filter subsection; `docs/RaceLink_Host/architecture.md` §"Cross-network fan-out" cascade rule |
