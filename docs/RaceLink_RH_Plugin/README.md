...
The dependency-format decision for `custom_plugins/racelink_rh_plugin/manifest.json` is documented in [docs/manifest-dependency-format.md](manifest-dependency-format.md).
...
- `uv` dependency on the immutable `racelink-host` GitHub release wheel for the repo-pinned development baseline `0.1.3`
...
The short maintainer playbook lives in [docs/release-playbook.md](release-playbook.md).

## License

Distributed under the **MIT** License. See [`LICENSE`](https://github.com/PSi86/RaceLink_RH-plugin/blob/main/LICENSE) for more information.
